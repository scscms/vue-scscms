    ...
    if (rows.length > 0) {
      ...
      if (rows.length > 0) {
        ...
        if (success) {
          ...
          let result = await sendEmail(data.user_email, common.web_name + '【帐号激活】', body)
          if (result) {
            await connection.end()
            ctx.body = {
              success: true,
  ...
  if (!err) {
    ...
    if (rows.length) {
      ...
      if (result.affectedRows === 1) {
        ...
        let result = await sendEmail(data.user_email, common.web_name + '【密码找回】', body)
        if (result) {
          await connection.end()
          ctx.body = {
            success: true,
...
function saveXMLToText (file, data) {
  return new Promise((resolve, reject) => {
    fs.writeFile(`${__dirname.replace('server', 'dist')}/xml/${file}.xml`, Buffer.from(data), {flag: 'w'}, function (err) {
      resolve(err)
    })
  })
}
