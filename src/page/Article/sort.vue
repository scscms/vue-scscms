...
export default {
  ...
  methods: {
    ...
    saveSort () {
      this.$refs.sortForm.validate(v => {
        if (v) {
          ...
          utils.ajax.call(this, '/updateSort', this.form, (data, err) => {
            this.visible = false
            this.loading = false
            if (!err) {
              let json = data.data
    ...
    renderContent (h, { node, data, store }) {
      ...
      return h('span', { style: { width: '100%' } }, [h('span', [h('span'), [node.label, h('span', {
        style: {
          color: '#999',
          marginLeft: '10px'
        }
      }, common.sort_type[node.data.sort_type])]]), h('span', {
        style: {
          float: 'right',
          margin: '-2px 10px'
        }
      }, [but('success', 'plus'), but('warning', 'edit'), but('danger', 'delete')])])
    }
  },
  mixins: [common.mixin]
}
