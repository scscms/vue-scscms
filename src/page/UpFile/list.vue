<template>
  <div class="grid-table">
    <el-button style="margin-bottom:5px" type="danger" @click="batchesDelFiles" :disabled="grade.delFile">批量删除
    </el-button>
  ...
  methods: {
    batchesDelFiles () {
      if (this.multipleSelection.length) {
        ...
        this.$confirm(`确定要批量删除文件和记录吗？`, '系统提醒', {
          ...
          utils.ajax.call(this, '/delFile', { id: ids, delRecord: true }, (d, err) => {
            if (!err) {
              this.ajaxData()
            }
          })
    ...
    columnFormatter (row, column) {
      ...
      if (key === 'file_size') {
        ...
        return h('div', [h('el-popover', {
          ...
          attrs: { src: row.full_path },
          on: {
            error: () => {
              row.full_path = self.errPic
            }
          }
        })]), h('div', {
        ...
        let but = (row, type, name) => {
          return h('el-button', {
            props: { size: 'small', disabled: self.grade.delFile || (type === 'delFile' && row.is_delete === 1) },
            on: {
              click: () => {
    ...
    headleClick (type, row) {
      this.$confirm(`确定要删除文件${type === 'delFile' ? '' : '和记录'}吗？`, '系统提醒', {
        ...
        utils.ajax.call(this, '/delFile', { id: row.id, delRecord: type === 'delFileAll' }, (d, err) => {
          if (!err) {
            if (type === 'delFileAll') {
              this.ajaxData()
            } else {
    ...
    ajaxData () {
      utils.ajax.call(this, '/listUpFile', this.search_data, (obj, err) => {
        if (!err) {
